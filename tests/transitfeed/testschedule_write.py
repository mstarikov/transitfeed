...
class ScheduleBuilderTestCase(util.TempFileTestCaseBase):
    ...
    def test_build_feed_with_utf8_names(self):
    ...
    def test_build_simple_feed(self):
    ...
    def test_route_id_conflict(self):
        problems = util.GetTestFailureProblemReporter(self)
        schedule = transitfeed.Schedule(problem_reporter=problems)
    ...
    def test_trip_id_conflict(self):
        problems = util.GetTestFailureProblemReporter(self)
        schedule = transitfeed.Schedule(problem_reporter=problems)
        service_period = schedule.GetDefaultServicePeriod()
        service_period.set_date_has_service("20070101")
...
class WriteSampleFeedTestCase(util.TempFileTestCaseBase):
    def assert_equal_time_string(self, a, b):
    ...
    def assert_equal_with_default(self, a, b, default):
    ...
    def run_test(self):
        ...
        for trip_id, stop_time_list in stop_time_data.items():
            ...
            for stop_time_entry, read_stoptime in zip(stop_time_list, read_stoptimes):
                ...
                self.assert_equal_time_string(arrival_time, read_stoptime.arrival_time)
                self.assert_equal_time_string(departure_time, read_stoptime.departure_time)
                self.assertEqual(shape_dist_traveled, read_stoptime.shape_dist_traveled)
                self.assert_equal_with_default(pickup_type, read_stoptime.pickup_type, 0)
                self.assert_equal_with_default(drop_off_type, read_stoptime.drop_off_type, 0)
                self.assert_equal_with_default(stop_headsign, read_stoptime.stop_headsign, '')
