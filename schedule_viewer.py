...
class ScheduleRequestHandler(BaseHTTPRequestHandler):
    def do_get(self):
        ...
        if m and m.group(1):
            try:
                f, mime_type = self.open_file(m.group(1))
...
def get_default_key_file_path():
    ...
    windows_base = find_py2_exe_base()
    if windows_base:
...
def main(request_handler=ScheduleRequestHandler):
    usage = \
