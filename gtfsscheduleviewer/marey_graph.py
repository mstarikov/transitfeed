...
class MareyGraph:
    ...
    _MAX_ZOOM = 5.0  # change docstring of change_scale_factor if this changes
    ...
    def draw(self, stoplist=None, triplist=None, height=520):
        ...
        if not self._cache or triplist or stoplist:
            ...
            self._decorators = []
            self._stations = self._build_stations(stoplist)
            self._cache = "%s %s %s %s" % (self._draw_box(),
                                           self._draw_hours(),
                                           self._draw_stations(),
                                           self._draw_trips(triplist))

        output = "%s %s %s %s" % (self._draw_header(),
                                  self._cache,
    ...
    def _build_stations(self, stoplist):
        ...
        """
        stations = []
        dists = self._euclidian_distances(stoplist)
        stations = self._calculate_y_lines(dists)
        return stations

    @staticmethod
    def _euclidian_distances(slist):
    ...
    def _travel_times(self, triplist, index=0):
    ...
    @staticmethod
    def _add_warning(str):
        print(str)

    def _draw_trips(self, triplist, colpar=""):
        ...
        if not self._stations and triplist:
            self._stations = self._calculate_y_lines(self._travel_times(triplist))
            if not self._stations:
                self._add_warning("Failed to use traveltimes for graph")
                self._stations = self._calculate_y_lines(self._uniform(triplist))
                if not self._stations:
    ...
    def add_trip_decoration(self, triplist, color="#f00"):
        ...
        Args:
          ...
          color: "#fff"
        """
        tmpstr = self._draw_trips(triplist, color)
        self._decorators.append(tmpstr)

    def change_scale_factor(self, newfactor):
        ...
        Args:
          ...
          newfactor: 0.7
        """
        if 0 < float(newfactor) < self._MAX_ZOOM:
